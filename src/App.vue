  ...
  methods: {
    ...
    subscribeToUpdates(tickerName) {
      let updaterInterval = setInterval(async () => {
        ...
        let thatTicker = this.tickers.find(t => t.name === tickerName);

        /** Заглушка */
        const data = { USD: Math.ceil(Math.random() * 100) };

        thatTicker.price =
          data.USD > 1 ? data.USD.toFixed(2) : data.USD.toPrecision(2);

        /* Записываем id интервала чтоб сбросить когда надо будет */
        thatTicker.updater = updaterInterval;
    ...
    handleDelete(tickerToRemove) {
      /* При удалении элемента останавливаем его обновление через setInterval */
      clearInterval(tickerToRemove.updater);
      this.tickers = this.tickers.filter(t => t !== tickerToRemove);
      /* Удаляем так же из локал стоража */
      localStorage.setItem("cryptonomicon-list", JSON.stringify(this.tickers));
    },
